...
func main() {
	...
	if options.ListDslSignatures {
		gologger.Info().Msgf("可用的自定义DSL函数有:")
	...
	if memProfile != "" {
		...
		if err != nil {
			gologger.Fatal().Msgf("无法创建内存文件 %q: %v", memProfile, err)
		}
		old := runtime.MemProfileRate
		runtime.MemProfileRate = 4096
		gologger.Print().Msgf("启用内存分析 (速 %d), %s", runtime.MemProfileRate, memProfile)

		defer func() {
			_ = pprof.Lookup("heap").WriteTo(f, 0)
			f.Close()
			runtime.MemProfileRate = old
			gologger.Print().Msgf("禁用内存分析，%s", memProfile)
		}()
	}
	...
	if err != nil {
		gologger.Fatal().Msgf("无法创建runner对象: %s\n", err)
	}
	if nucleiRunner == nil {
		return
	}
	...
	go func() {
		for range c {
			gologger.Info().Msgf("CTRL+C 按下：正在退出\n")
			nucleiRunner.Close()
			if options.ShouldSaveResume() {
				gologger.Info().Msgf("正在创建恢复文件: %s\n", resumeFileName)
				err := nucleiRunner.SaveResumeConfig(resumeFileName)
				if err != nil {
					gologger.Error().Msgf("无法创建恢复文件: %s\n", err)
				}
			}
			os.Exit(1)
		}
	...
	if err := nucleiRunner.RunEnumeration(); err != nil {
		if options.Validate {
			gologger.Fatal().Msgf("不能验证扫描模板: %s\n", err)
		} else {
			gologger.Fatal().Msgf("不能运行 nuclei: %s\n", err)
		}
	}
	nucleiRunner.Close()
 ...
 ▀███▀███▀   ▀██████▀   ▀██████▀  █████▄▄██ ████████▀   ▀██████▀
	...
	*/
	...
	flagSet.CreateGroup("output", "Output",
		...
		flagSet.BoolVarP(&options.JSONL, "jsonl", "j", false, "以JSONL(ines)格式输出"),
		...
		flagSet.StringVarP(&options.JSONExport, "json-export", "je", "", "要以JSON格式导出结果的文件"),
		flagSet.StringVarP(&options.JSONLExport, "jsonl-export", "jle", "", "要以JSONL(ine)格式导出结果的文件"),
	)
	...
	flagSet.CreateGroup("optimization", "Optimizations",
		flagSet.IntVar(&options.Timeout, "timeout", 10, "超时前等待的时间（秒）"),
		...
		flagSet.BoolVar(&options.Stream, "stream", false, "在不排序输入的情况下开始细化"),
	...
	flagSet.CreateGroup("debug", "Debug",
		...
		flagSet.CallbackVar(printVersion, "version", "显示nuclei的版本"),
		...
		flagSet.CallbackVarP(printTemplateVersion, "templates-version", "tv", "显示已安装的Nuclei模板的版本"),
		flagSet.BoolVarP(&options.HealthCheck, "health-check", "hc", false, "运行诊断检查"),
	)

	flagSet.CreateGroup("update", "Update",
		flagSet.CallbackVarP(runner.NucleiToolUpdateCallback, "update", "up", "将Nuclei引擎更新至最新发布版本"),
		flagSet.BoolVarP(&options.UpdateTemplates, "update-templates", "ut", false, "将Nuclei规则模板更新至最新发布版本"),
		flagSet.StringVarP(&options.NewTemplatesDirectory, "update-template-dir", "ud", "", "覆盖默认目录以安装模板"),
		flagSet.CallbackVarP(disableUpdatesCallback, "disable-update-check", "duc", "禁用自动nuclei/模板更新检查"),
	)
	...
	if cfgFile != "" {
		if !fileutil.FileExists(cfgFile) {
			gologger.Fatal().Msgf("提供配置文件 '%s' 不存在", cfgFile)
		}
...
func readFlagsConfig(flagset *goflags.FlagSet) {
	...
	if err != nil {
		...
		gologger.Warning().Msgf("不能读取配置文件 : %s\n", err)
		return
	}
...
func printVersion() {
	gologger.Info().Msgf("Nuclei的引擎版本: %s", config.Version)
	os.Exit(0)
}
